...
public class WebWorld extends World
{
    ...
    public void terrainRender(SpriteRenderer renderer)
    {
        ...
        for (int i = 0; i < this.getWorldHeight(); i++)
        {
            ...
            renderer.draw(Sprite.WALL, this.getWorldHeight() - 1, i);
            renderer.draw(Sprite.WALL, i, 0);
            renderer.draw(Sprite.WALL, i, this.getWorldHeight() - 1);
        }
        // Draw the floor and interior walls
        for (int i = 1; i < this.getWorldHeight() - 1; i++)
        {
            for (int j = 1; j < this.getWorldHeight() - 1; j++)
            {
