...
//	private static final String	JSON_URL			= "snap1f.json";
	...
	{
		String fileName = Window.Location.getParameter("data");
		String dataURL = ( fileName == null ? "snap1" : fileName ) + ".json";
		...
		try
		{
			builder.sendRequest(null, new RequestCallback()
			{
				...
				{
					Window.alert("Couldn't retrieve JSON cow");
				}

				public void onResponseReceived(Request request, Response response)
				{
					world = JSONDecoder.convertJSONToWorld(response.getText());
					displayWorld();
				}
			});
		}
		...
		{
			Window.alert("Couldn't retrieve JSON duck");
		}
		// world = new WebWorld(10);
		// s displayWorld();

		/*
		 * _webService.getWorldOfSize(10, new AsyncCallback<WebWorld>() { public
		 * void onFailure(Throwable caught) { // TODO
		 * Window.alert("Dang gal, this also dun work at all! "+caught); }
		 * 
		 * public void onSuccess(WebWorld result) { // Make a new world with a
		 * world = result; displayWorld(); } });
		 */

		/*
		 * _webService.getWorldSize(new AsyncCallback<Integer>() { public void
		 * onFailure(Throwable caught) { // TODO
		 * Window.alert("Dang son, this dun work at all!"); }
		 * 
		 * public void onSuccess(Integer result) { // Make a new world with a
		 * world = new WebWorld(result); displayWorld(); } });
		 */
	...
	private void displayWorld()
	{
		...
		canvas.setCoordinateSpaceWidth(world.getWorldDimensions() * VivariumWeb.PIXEL_BLOCK_SIZE);
		canvas.setCoordinateSpaceHeight(world.getWorldDimensions() * VivariumWeb.PIXEL_BLOCK_SIZE);
