...
public class BrainTest
{
    ...
    public void validateWeights()
    {
        ...
        NeuralNetworkBrain brain = new NeuralNetworkBrain(0, 0, 0, false);
        Tester.isTrue("Validate a brand new Creature initialized with all zeros",
                brain.validateWeights(brain.getWeights()));
        ...
        Tester.isFalse("One layer empty matrix doesn't validate for zero input/output brain",
                brain.validateWeights(oneLayerEmpty));
        ...
        Tester.isTrue("Matrix with only one vector will compute a single output",
                brain.validateWeights(new double[][][] { { { 0, 0 } } }));

        brain.setInputCount(1);
        Tester.isTrue("Input = 1, Output = 2, Single Layer Weight Size is 1x3",
                brain.validateWeights(new double[][][] { { { 0, 0, 1 } } }));
        Tester.isFalse("Matrix with insufficient single vector length fails",
                brain.validateWeights(new double[][][] { { { 0 } } }));

        brain.setOutputCount(2);
        Tester.isTrue("Input = 1, Output = 2, Single Layer Weight Size is 2x3",
                brain.validateWeights(new double[][][] { { { 0, 0, 1 }, { 1, 1, 0 } } }));
        Tester.isFalse("Input = 1, Output = 2, Single Layer Weight Size is irregular",
                brain.validateWeights(new double[][][] { { { 0, 0 }, { 1, 1, 0 } } }));
        Tester.isFalse("Input = 1, Output = 2, Single Layer Weight Size is irregular",
                brain.validateWeights(new double[][][] { { { 0, 0, 1 }, { 1, 1 } } }));
    }
}
