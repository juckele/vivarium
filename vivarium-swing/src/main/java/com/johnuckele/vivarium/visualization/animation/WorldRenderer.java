...
public class WorldRenderer {
    ...
    public static void terrainRender(Graphics2D g2, World w,
	    ImageObserver observer) {
	// Draw the exterior walls
	for (int i = 0; i < w.getWorldDimensions(); i++) {
	    SpriteRenderer.drawSprite(g2, Sprite.WALL, i, 0, observer);
	    SpriteRenderer.drawSprite(g2, Sprite.WALL, i,
		    w.getWorldDimensions() - 1, observer);
	    SpriteRenderer.drawSprite(g2, Sprite.WALL, 0, i, observer);
	    SpriteRenderer.drawSprite(g2, Sprite.WALL,
		    w.getWorldDimensions() - 1, i, observer);
	}
    ...
    public static void actorRender(Graphics2D g2, World w,
	    ImageObserver observer, int milliseconds) {
	...
	for (int i = 1; i < w.getWorldDimensions() - 1; i++) {
		...
		if (w.getEntityType(i, j) == EntityType.FOOD) {
		    ...
		    int offsetMilliseconds = (int) (milliseconds
			    + creature.getRandomSeed() * 1000) % 1000;
		    ...
		    SpriteRenderer.drawSprite(g2, creatureSprites, j, i,
			    creature.getFacing(), observer);
		}
	    }
	}
    }
